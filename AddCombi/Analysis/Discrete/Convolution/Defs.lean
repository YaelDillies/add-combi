...
* `Analysis.Discrete.conv`: Discrete convolution of two functions
* `Analysis.Discrete.diffConv`: Discrete difference convolution of two functions
* `Analysis.Discrete.iterConv`: Iterated convolution of a function
...
Note that `MeasureTheory.convolution`
...
/-- The trivial character, the unique function `e` such that `e ∗ f = f ∗ e = f` for all `f`. -/
...
/-- Difference convolution -/
def diffConv (f g : G → R) : G → R := fun a ↦ ∑ x : G × G with x.1 - x.2 = a, f x.1 * conj g x.2

infixl:71 " ○ " => diffConv

lemma diffConv_apply (f g : G → R) (a : G) :
    (f ○ g) a = ∑ x : G × G with x.1 - x.2 = a , f x.1 * conj g x.2 := rfl

@[simp] lemma diffConv_zero (f : G → R) : f ○ 0 = 0 := by ext; simp [diffConv_apply]
@[simp] lemma zero_diffConv (f : G → R) : 0 ○ f = 0 := by ext; simp [diffConv_apply]

lemma diffConv_add (f g h : G → R) : f ○ (g + h) = f ○ g + f ○ h := by
  ext; simp [diffConv_apply, mul_add, sum_add_distrib]

lemma add_diffConv (f g h : G → R) : (f + g) ○ h = f ○ h + g ○ h := by
  ext; simp [diffConv_apply, add_mul, sum_add_distrib]

lemma smul_diffConv [DistribSMul H R] [IsScalarTower H R R] (c : H) (f g : G → R) :
    c • f ○ g = c • (f ○ g) := by ext; simp [diffConv_apply, smul_sum, smul_mul_assoc]

lemma diffConv_smul [Star H] [DistribSMul H R] [SMulCommClass H R R] [StarModule H R] (c : H)
    (f g : G → R) : f ○ c • g = star c • (f ○ g) := by
  ext; simp [diffConv_apply, smul_sum, mul_smul_comm, starRingEnd_apply, star_smul]

@[simp] lemma translate_diffConv (a : G) (f g : G → R) : τ a f ○ g = τ a (f ○ g) :=
  funext fun b ↦ sum_equiv ((Equiv.subRight a).prodCongr $ Equiv.refl _)
    (by simp [sub_right_comm _ a]) (by simp)

@[simp] lemma diffConv_translate (a : G) (f g : G → R) : f ○ τ a g = τ (-a) (f ○ g) :=
  funext fun b ↦ sum_equiv ((Equiv.refl _).prodCongr $ Equiv.subRight a)
...
@[simp] lemma diffConv_conjneg (f g : G → R) : f ○ conjneg g = f ∗ g := by
...
@[simp] lemma conj_diffConv_apply (f g : G → R) (a : G) : conj ((f ○ g) a) = (conj f ○ conj g) a := by
...
@[simp] lemma conj_diffConv (f g : G → R) : conj (f ○ g) = conj f ○ conj g :=
  funext <| conj_diffConv_apply _ _

lemma IsSelfAdjoint.conv (hf : IsSelfAdjoint f) (hg : IsSelfAdjoint g) : IsSelfAdjoint (f ∗ g) :=
  (conj_conv _ _).trans $ congr_arg₂ _ hf hg

lemma IsSelfAdjoint.diffConv (hf : IsSelfAdjoint f) (hg : IsSelfAdjoint g) : IsSelfAdjoint (f ○ g) :=
  (conj_diffConv _ _).trans $ congr_arg₂ _ hf hg
...
@[simp] lemma conjneg_diffConv (f g : G → R) : conjneg (f ○ g) = g ○ f := by
  simp_rw [← conv_conjneg, conjneg_conv, conjneg_conjneg, conv_comm]
alias smul_diffConv_assoc := smul_diffConv
alias smul_diffConv_left_comm := diffConv_smul

lemma diffConv_right_comm (f g h : G → R) : f ○ g ○ h = f ○ h ○ g := by
  simp_rw [← conv_conjneg, conv_right_comm]

lemma conv_diffConv_assoc (f g h : G → R) : f ∗ g ○ h = f ∗ (g ○ h) := by
  simp_rw [← conv_conjneg, conv_assoc]

lemma conv_diffConv_left_comm (f g h : G → R) : f ∗ (g ○ h) = g ∗ (f ○ h) := by
  simp_rw [← conv_conjneg, conv_left_comm]

lemma conv_diffConv_right_comm (f g h : G → R) : f ∗ g ○ h = f ○ h ∗ g := by
  simp_rw [← conv_conjneg, conv_right_comm]

lemma conv_diffConv_conv_comm (f g h i : G → R) : f ∗ g ○ (h ∗ i) = f ○ h ∗ (g ○ i) := by
  simp_rw [← conv_conjneg, conjneg_conv, conv_conv_conv_comm]

lemma diffConv_conv_diffConv_comm (f g h i : G → R) : f ○ g ∗ (h ○ i) = f ∗ h ○ (g ∗ i) := by
  simp_rw [← conv_conjneg, conjneg_conv, conv_conv_conv_comm]

lemma diffConv_diffConv_diffConv_comm (f g h i : G → R) : f ○ g ○ (h ○ i) = f ○ h ○ (g ○ i) := by
  simp_rw [← conv_conjneg, conjneg_conv, conv_conv_conv_comm]

--TODO: Can we generalise to star ring homs?
lemma map_diffConv (f g : G → ℝ≥0) (a : G) : (↑((f ○ g) a) : ℝ) = ((↑) ∘ f ○ (↑) ∘ g) a := by
  simp_rw [diffConv_apply, NNReal.coe_sum, NNReal.coe_mul, starRingEnd_apply, star_trivial,
    Function.comp_apply]

lemma comp_diffConv (f g : G → ℝ≥0) : ((↑) ∘ (f ○ g) : G → ℝ) = (↑) ∘ f ○ (↑) ∘ g :=
  funext $ map_diffConv _ _

lemma diffConv_eq_sum_sub (f g : G → R) (a : G) : (f ○ g) a = ∑ t, f (a - t) * conj (g (-t)) := by
  simp [← conv_conjneg, conv_eq_sum_sub]

lemma diffConv_eq_sum_add (f g : G → R) (a : G) : (f ○ g) a = ∑ t, f (a + t) * conj (g t) := by
  simp [← conv_conjneg, conv_eq_sum_add]

lemma diffConv_eq_sum_sub' (f g : G → R) (a : G) : (f ○ g) a = ∑ t, f t * conj (g (t - a)) := by
  simp [← conv_conjneg, conv_eq_sum_sub']

lemma diffConv_eq_sum_add' (f g : G → R) (a : G) : (f ○ g) a = ∑ t, f (-t) * conj g (-(a + t)) := by
  simp [← conv_conjneg, conv_eq_sum_add']

lemma diffConv_apply_neg (f g : G → R) (a : G) : (f ○ g) (-a) = conj ((g ○ f) a) := by
  rw [← conjneg_diffConv f, conjneg_apply, Complex.conj_conj]

lemma diffConv_apply_sub (f g : G → R) (a b : G) :
    (f ○ g) (a - b) = ∑ t, f (a + t) * conj (g (b + t)) := by
  simp [← conv_conjneg, sub_eq_add_neg, conv_apply_add, add_comm]

lemma sum_diffConv_mul (f g h : G → R) :
    ∑ a, (f ○ g) a * h a = ∑ a, ∑ b, f a * conj (g b) * h (a - b) := by
  simp_rw [diffConv_eq_sum_sub', sum_mul]
  rw [sum_comm]
  exact Fintype.sum_congr _ _ fun x ↦ Fintype.sum_equiv (Equiv.subLeft x) _ _ fun y ↦ by simp

lemma sum_diffConv (f g : G → R) : ∑ a, (f ○ g) a = (∑ a, f a) * ∑ a, conj (g a) := by
  simpa only [Fintype.sum_mul_sum, Pi.one_apply, mul_one] using sum_diffConv_mul f g 1

@[simp] lemma diffConv_const (f : G → R) (b : R) : f ○ const _ b = const _ ((∑ x, f x) * conj b) := by
  ext; simp [diffConv_eq_sum_sub', sum_mul]

@[simp] lemma const_diffConv (b : R) (f : G → R) : const _ b ○ f = const _ (b * ∑ x, conj (f x)) := by
  ext; simp [diffConv_eq_sum_add, mul_sum]

@[simp] lemma diffConv_trivChar (f : G → R) : f ○ trivChar = f := by ext a; simp [diffConv_eq_sum_add]
@[simp] lemma trivChar_diffConv (f : G → R) : trivChar ○ f = conjneg f := by
  rw [← conv_conjneg, trivChar_conv]

lemma support_diffConv_subset (f g : G → R) : support (f ○ g) ⊆ support f - support g := by
...
@[simp] lemma diffConv_neg (f g : G → R) : f ○ -g = -(f ○ g) := by ext; simp [diffConv_apply]
@[simp] lemma neg_diffConv (f g : G → R) : -f ○ g = -(f ○ g) := by ext; simp [diffConv_apply]

lemma diffConv_sub (f g h : G → R) : f ○ (g - h) = f ○ g - f ○ h := by
  simp only [sub_eq_add_neg, diffConv_add, diffConv_neg]

lemma sub_diffConv (f g h : G → R) : (f - g) ○ h = f ○ h - g ○ h := by
  simp only [sub_eq_add_neg, add_diffConv, neg_diffConv]
...
@[simp, norm_cast]
lemma coe_diffConv : (↑((f ○ g) a) : 𝕜) = ((↑) ∘ f ○ (↑) ∘ g) a := by simp [diffConv_apply, coe_sum]

@[simp] lemma coe_comp_conv : ((↑) : ℝ → 𝕜) ∘ (f ∗ g) = (↑) ∘ f ∗ (↑) ∘ g := funext $ coe_conv _ _
@[simp] lemma coe_comp_diffConv : ((↑) : ℝ → 𝕜) ∘ (f ○ g) = (↑) ∘ f ○ (↑) ∘ g := funext $ coe_diffConv _ _
...
@[simp, norm_cast]
lemma ofReal_diffConv : (↑((f ○ g) a) : ℂ) = ((↑) ∘ f ○ (↑) ∘ g) a := RCLike.coe_diffConv _ _ _

@[simp] lemma ofReal_comp_conv : ((↑) : ℝ → ℂ) ∘ (f ∗ g) = (↑) ∘ f ∗ (↑) ∘ g :=
  funext $ ofReal_conv _ _

@[simp] lemma ofReal_comp_diffConv : ((↑) : ℝ → ℂ) ∘ (f ○ g) = (↑) ∘ f ○ (↑) ∘ g :=
  funext $ ofReal_diffConv _ _
...
@[simp, norm_cast]
lemma coe_diffConv : (↑((f ○ g) a) : ℝ) = ((↑) ∘ f ○ (↑) ∘ g) a := by simp [diffConv_apply, coe_sum]

@[simp] lemma coe_comp_conv : ((↑) : _ → ℝ) ∘ (f ∗ g) = (↑) ∘ f ∗ (↑) ∘ g :=
  funext $ coe_conv _ _

@[simp] lemma coe_comp_diffConv : ((↑) : _ → ℝ) ∘ (f ○ g) = (↑) ∘ f ○ (↑) ∘ g :=
  funext $ coe_diffConv _ _
...
lemma iterConv_diffConv_distrib (f g : G → R) : ∀ n, (f ○ g) ∗^ n = f ∗^ n ○ g ∗^ n
  | 0 => (diffConv_trivChar _).symm
  | n + 1 => by simp_rw [iterConv_succ, iterConv_diffConv_distrib, conv_diffConv_conv_comm]
...
@[simp, norm_cast]
lemma nnrealOfReal_iterConv (f : G → ℝ≥0) (n : ℕ) (a : G) : (↑((f ∗^ n) a) : ℝ) = ((↑) ∘ f ∗^ n) a :=
