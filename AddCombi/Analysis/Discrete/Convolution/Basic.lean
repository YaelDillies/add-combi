...
* `diffConv`: Discrete difference convolution of two functions
...
lemma indicate_diffConv_indicate_apply (s t : Finset G) (a : G) :
    (𝟭_[R] s ○ 𝟭 t) a = #{x ∈ s ×ˢ t | x.1 - x.2 = a} := by
  simp only [diffConv_apply, indicate_apply, ← ite_and, filter_comm, boole_mul, sum_boole,
    apply_ite conj, map_one, map_zero, Pi.conj_apply]
  simp_rw [← mem_product, filter_univ_mem]

lemma indicate_diffConv (s : Finset G) (f : G → R) : 𝟭 s ○ f = ∑ a ∈ s, τ a (conjneg f) := by
  ext; simp [diffConv_eq_sum_sub', indicate_apply]

lemma diffConv_indicate (f : G → R) (s : Finset G) : f ○ 𝟭 s = ∑ a ∈ s, τ (-a) f := by
  ext; simp [diffConv_eq_sum_add, indicate_apply]
...
@[simp] lemma mu_univ_diffConv_mu_univ : μ_[R] (univ : Finset G) ○ μ univ = μ univ := by
  ext; cases eq_or_ne (card G : R) 0 <;> simp [mu_apply, diffConv_eq_sum_add, card_univ, *]

lemma mu_diffConv (s : Finset G) (f : G → R) :
    μ s ○ f = (#s : R)⁻¹ • ∑ a ∈ s, τ a (conjneg f) := by
  simp [mu, indicate_diffConv, smul_diffConv]

lemma diffConv_mu (f : G → R) (s : Finset G) : f ○ μ s = (#s : R)⁻¹ • ∑ a ∈ s, τ (-a) f := by
  simp [mu, diffConv_indicate, diffConv_smul]
...
lemma expect_diffConv (f g : G → R) : 𝔼 a, (f ○ g) a = (∑ a, f a) * 𝔼 a, conj (g a) := by
  simp_rw [expect, sum_diffConv, mul_smul_comm]

lemma expect_diffConv' (f g : G → R) : 𝔼 a, (f ○ g) a = (𝔼 a, f a) * ∑ a, conj (g a) := by
  simp_rw [expect, sum_diffConv, smul_mul_assoc]
...
@[simp] lemma balance_diffConv (f g : G → R) : balance (f ○ g) = balance f ○ balance g := by
  simpa [balance, diffConv_sub, sub_diffConv, expect_diffConv, map_expect]
    using (mul_smul_comm _ _ _).trans (smul_mul_assoc _ _ _).symm
...
lemma indicate_iterConv_diffConv (s : Finset G) (n : ℕ) (f : G → R) :
    𝟭 s ∗^ n ○ f = ∑ a ∈ s ^^ n, τ (∑ i, a i) (conjneg f) := by
  rw [← conv_conjneg, indicate_iterConv_conv]

lemma diffConv_indicate_iterConv (f : G → R) (s : Finset G) (n : ℕ) :
    f ○ 𝟭 s ∗^ n = ∑ a ∈ s ^^ n, τ (-∑ i, a i) f := by
...
lemma mu_iterConv_diffConv (s : Finset G) (n : ℕ) (f : G → R) :
    μ s ∗^ n ○ f = 𝔼 a ∈ piFinset (fun _ : Fin n ↦ s), τ (∑ i, a i) (conjneg f) := by
  rw [← conv_conjneg, mu_iterConv_conv]

lemma diffConv_mu_iterConv (f : G → R) (s : Finset G) (n : ℕ) :
    f ○ μ s ∗^ n = 𝔼 a ∈ piFinset (fun _ : Fin n ↦ s), τ (-∑ i, a i) f := by
