...
lemma thing_one : (𝟭_[R] B ○ 𝟭 A) x = ∑ y, 𝟭 A y * 𝟭 B (x + y) := by
  simp only [diffConv_eq_sum_add, conj_indicate_apply, mul_comm]

lemma thing_one_right : (𝟭_[R] A ○ 𝟭 B) x = #(A ∩ (x +ᵥ B)) := by
  rw [indicate_diffConv_indicate_apply]
  congr 1
  apply card_nbij' Prod.fst (fun a ↦ (a, a - x)) <;> aesop (add simp [mem_vadd_finset])

lemma thing_two : ∑ s, (𝟭_[R] A ○ 𝟭 B) s = #A * #B := by
  simp only [sum_diffConv, conj_indicate_apply, sum_indicate]

lemma thing_three : ∑ s, ((𝟭 A ○ 𝟭 B) s ^ 2 : R) = E[A, B] := by
  simp only [indicate_diffConv_indicate_apply, card_eq_sum_ones, Nat.cast_sum, Nat.cast_one, sum_mul,
...
lemma claim_one : ∑ s, (𝟭_[R] A ○ 𝟭 B) s * #(A ∩ (s +ᵥ B)) = E[A, B] := by
  simp only [← thing_three, ← thing_one_right, sq]

lemma claim_two :
    (E[A, B]) ^ 2 / (#A * #B) ≤ ∑ s, (𝟭_[ℝ] A ○ 𝟭 B) s * #(A ∩ (s +ᵥ B)) ^ 2 := by
  let f := fun s ↦ ((𝟭_[ℝ] A ○ 𝟭 B) s).sqrt
  have hf : ∀ s, f s ^ 2 = (𝟭_[ℝ] A ○ 𝟭 B) s := by
    intro s
    rw [Real.sq_sqrt]
    exact diffConv_nonneg (R := ℝ) indicate_nonneg indicate_nonneg s -- why do I need the annotation??
  ...
  · refine sum_nonneg fun i _ ↦ ?_
    ...
    exact mul_nonneg (diffConv_nonneg indicate_nonneg indicate_nonneg _) (by positivity)
...
lemma claim_three {H : Finset (G × G)} (hH : H ⊆ A ×ˢ A) :
    ∑ s, (𝟭_[ℝ] A ○ 𝟭 B) s * #((A ∩ (s +ᵥ B)) ×ˢ (A ∩ (s +ᵥ B)) ∩ H) =
      ∑ ab ∈ H, ∑ s, (𝟭_[ℝ] A ○ 𝟭 B) s * (𝟭 B (ab.1 - s) * 𝟭 B (ab.2 - s)) := by
...
lemma claim_four (ab : G × G) :
  ...
  rcases ab with ⟨a, b⟩
  have : ∀ s, (𝟭_[ℝ] A ○ 𝟭 B) s ≤ #B := fun s ↦ by
    simp only [diffConv_eq_sum_add, conj_indicate_apply, card_eq_sum_ones, Nat.cast_sum, Nat.cast_one]
...
lemma claim_eight (c : ℝ) (hc : 0 ≤ c) (hA : (0 : ℝ) < #A) (hB : (0 : ℝ) < #B) :
  ...
  · intros s _
    exact mul_le_mul_of_nonneg_left (this s).le (diffConv_nonneg indicate_nonneg indicate_nonneg s)
  have : 0 < 𝟭_[ℝ] A ○ 𝟭 B := by
    refine diffConv_pos ?_ ?_
