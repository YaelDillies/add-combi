...
for file in files:
    ...
    for imp in header:
        ...
        if (not imp_file.startswith("Mathlib.")) and \
            (not imp_file.startswith("AddCombi.Mathlib")):
            raise OSError(f"""
MathlibImportError: A file in AddCombi.Mathlib imports a file not in Mathlib or AddCombi.Mathlib
\tImporting file: {file}
